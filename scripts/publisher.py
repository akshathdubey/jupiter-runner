...
def decrypt(value: str) -> str:
    ...
    return AESGCM(key).decrypt(iv, raw_cipher if False else cipher + tag, None).decode("utf-8")
...
def youtube_upload(token: str, content: dict, metadata: dict, video_bytes: bytes) -> str:
    ...
    body = {"snippet": {"title": title, "description": description, "tags": tags, "categoryId": "27"}, "status": {"privacyStatus": "private", "selfDeclaredMadeForKids": False}}
    init = requests.post("https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status", headers=headers, json=body, timeout=30)
...
def facebook_publish(token: str, content: dict, metadata: dict, video_bytes: bytes) -> str:
    ...
    payload = {"description": str(metadata.get("description") or content.get("prompt", "")), "access_token": page["access_token"]}
    response = requests.post(f"https://graph.facebook.com/{META_VERSION}/{page['id']}/videos", data=payload, files=files, timeout=20 * 60)
    response.raise_for_status()
...
def main():
    ...
    supabase.table("publication_jobs").update({"status": "published", "published_at": "now()", "platform_post_id": post_id}).eq("id", JOB_ID).execute()
