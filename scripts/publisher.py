...
def decrypt(value: str) -> str:
    ...
    if len(key) != 32:
        raise RuntimeError("JUPITER_TOKEN_ENCRYPTION_KEY must decode to 32 bytes.")
...
def get_job():
    ...
    if not row:
        raise RuntimeError("Publication job not found.")
    return row
...
def get_content(job):
    ...
    if not row:
        raise RuntimeError("Content job not found.")
    return row
...
def get_connection(job):
    ...
    if not row:
        raise RuntimeError("Social connection not found.")
    return row
...
def youtube_upload(token: str, content: dict, metadata: dict, video_bytes: bytes) -> str:
    ...
    # Automatic/manual publication both mean "publish now" once this worker is invoked.
    # Users can choose platform targets and publication mode before the worker is dispatched.
    privacy = str((metadata.get("platform_metadata") or {}).get("youtube", {}).get("privacy_status") or "public").lower()
    if privacy not in {"public", "private", "unlisted"}:
        privacy = "public"
    init = requests.post(
        "https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status",
        headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json", "X-Upload-Content-Type": "video/mp4", "X-Upload-Content-Length": str(len(video_bytes))},
        json={"snippet": {"title": title, "description": description, "tags": tags, "categoryId": "27"}, "status": {"privacyStatus": privacy, "selfDeclaredMadeForKids": False}},
        timeout=30,
    )
...
def meta_pages(token: str):
    ...
    if not data:
        raise RuntimeError("No Facebook Page is available to publish to.")
    return data[0]


def facebook_publish(token: str, content: dict, metadata: dict, video_bytes: bytes) -> str:
    page = meta_pages(token)
    description = str(metadata.get("description") or content.get("prompt", ""))
    response = requests.post(
        f"https://graph.facebook.com/{META_VERSION}/{page['id']}/videos",
        data={"description": description, "access_token": page["access_token"]},
        files={"source": ("jupiter.mp4", video_bytes, "video/mp4")},
        timeout=20 * 60,
    )
    response.raise_for_status()
...
def instagram_publish(token: str, content: dict, metadata: dict, video_url: str) -> str:
    ...
    if not ig:
        raise RuntimeError("No Instagram Business account is linked to the selected Facebook Page.")
    ...
    return str(publish.json().get("id"))


def reconcile_content_job(content_job_id: str) -> None:
    rows = supabase.table("publication_jobs").select("status").eq("content_job_id", content_job_id).execute().data or []
    if not rows:
        return
    statuses = {str(row.get("status")) for row in rows}
    if statuses and statuses.issubset({"published"}):
        supabase.table("content_jobs").update({"status": "published", "stage": "published", "progress": 100, "completed_at": datetime.now(timezone.utc).isoformat(), "error": None}).eq("id", content_job_id).execute()
    elif "failed" in statuses and statuses.issubset({"published", "failed"}):
        supabase.table("content_jobs").update({"status": "partially_published", "stage": "partially_published", "progress": 100}).eq("id", content_job_id).execute()
    else:
        supabase.table("content_jobs").update({"status": "publishing", "stage": "publishing", "progress": 98}).eq("id", content_job_id).execute()


def main():
    job = get_job()
    content = get_content(job)
    ...
    try:
        ...
        video_bytes = download_video(content["output_path"])
        seo = supabase.table("seo_metadata").select("*").eq("content_job_id", content["id"]).single().execute().data or {}
        platform = job["platform"]
        if platform == "youtube":
            post_id = youtube_upload(token, content, seo, video_bytes)
        elif platform == "facebook":
            post_id = facebook_publish(token, content, seo, video_bytes)
            ...
            if not video_url:
                raise RuntimeError("Could not create a temporary video URL for Instagram.")
            post_id = instagram_publish(token, content, seo, video_url)
        else:
            raise RuntimeError(f"Unsupported platform: {platform}")
        supabase.table("publication_jobs").update({"status": "published", "published_at": datetime.now(timezone.utc).isoformat(), "platform_post_id": post_id, "error": None}).eq("id", JOB_ID).execute()
        reconcile_content_job(content["id"])
        print(f"PUBLISHED {platform} {post_id}")
    except Exception as exc:
        supabase.table("publication_jobs").update({"status": "failed", "error": str(exc)}).eq("id", JOB_ID).execute()
        reconcile_content_job(content["id"])
        raise
