...
def watermark_video(input_path: Path, output_path: Path) -> None:
    ...
    result = subprocess.run(
        [ffmpeg, "-y", "-hide_banner", "-loglevel", "error", "-i", str(input_path), "-vf", vf,
         "-c:v", "libx264", "-preset", "veryfast", "-crf", "23", "-pix_fmt", "yuv420p",
         "-c:a", "aac", "-b:a", "128k", "-t", str(PREVIEW_SECONDS), "-movflags", "+faststart", str(output_path)],
...
def main() -> None:
    job = get_job()
    update_job(status="running", stage="preview_ingesting", progress=5, started_at=datetime.now(timezone.utc).isoformat(), error=None, preview_status="running")
    with tempfile.TemporaryDirectory(prefix=f"jupiter-preview-{JOB_ID}-") as tmp:
        ...
        package = build_short_package(prompt=str(job["prompt"]), source_context=source_context, target_seconds=PREVIEW_SECONDS, tone=str(job.get("tone") or "infotainment"), quality=QUALITY)
        ...
        result = generate_final_video(teacher, visual, rendered, quality="normal", tts_voice=str(job.get("narrator") or "en-IN-NeerjaNeural"), tts_rate="+0%", tts_volume="+0%", max_repair_cycles=1, render_timeout_seconds=600)
...
if __name__ == "__main__":
    try:
        ...
        try:
            update_job(status="failed", stage="preview_error", progress=100, preview_status="failed", error=str(exc))
        finally:
            raise
