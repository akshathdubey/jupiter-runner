...
def multipart_upload(url: str, fields: dict[str, str], file_field: str, filename: str, data: bytes, content_type: str) -> dict:
...
def wait_for_job(job_id: str, timeout_seconds: int, label: str) -> dict:
    ...
    while time.time() < deadline:
        ...
        if status in {"failed", "cancelled"}:
            raise RuntimeError(f"{label} job failed: stage={stage} error={job.get('error')}")
        time.sleep(10)
...
def create_fixture() -> bytes:
    ...
    pages = [
        (
            "Vision Transformers — E2E Fixture",
            [
                "A Vision Transformer (ViT) applies the transformer architecture to images.",
                "The image is split into fixed-size patches, and each patch becomes a token.",
                "A learnable class token can summarize the image for classification.",
            ],
        ),
        (
            "Patch Embeddings and Attention",
            [
                "Patch embeddings map image patches into a common vector space.",
                "Self-attention lets every patch interact with every other patch.",
                "Multi-head attention captures different relationships between visual tokens.",
            ],
        ),
        (
            "Why the Transformer Works",
            [
                "Positional information is added so token order and spatial structure are retained.",
                "Repeated transformer blocks refine representations through attention and MLP layers.",
                "The final representation can be used for supervised image classification.",
            ],
        ),
    ]
    for title, lines in pages:
        pdf.setFont("Helvetica-Bold", 22)
        pdf.drawString(54, height - 70, title)
        ...
        pdf.drawString(54, 54, "Jupiter autonomous E2E workflow #1")
        pdf.showPage()
    pdf.save()
    data = FIXTURE.read_bytes()
    if len(data) < 1000:
        raise RuntimeError("Generated E2E PDF fixture is unexpectedly small.")
    return data
...
def start_analysis(fixture: bytes) -> str:
    ...
    if not job_id:
        raise RuntimeError(f"Analysis API did not return a job_id: {response}")
...
def load_analysis(job: dict) -> dict:
    ...
    raw = supabase.storage.from_(BUCKET).download(result_path)
    analysis = json.loads(raw.decode("utf-8"))
    if analysis.get("status") != "ready_for_generation":
        raise RuntimeError(f"Analysis artifact is not generation-ready: {analysis.get('status')}")
    if (analysis.get("visual_validation") or {}).get("passed") is not True:
        raise RuntimeError("Analysis visual validation did not pass.")
    for key in ("teacher", "visual_design", "blueprint", "pricing"):
        if not analysis.get(key):
            raise RuntimeError(f"Analysis artifact is missing {key}.")
    return analysis


def create_render_job(analysis_job_id: str, analysis: dict) -> tuple[str, list[str]]:
    ...
    if getattr(created, "error", None):
        raise RuntimeError(f"Could not create E2E video job: {created.error}")
    ...
    for path, value in ((teacher_path, analysis["teacher"]), (visual_path, analysis["visual_design"])):
        ...
        if upload.error:
            raise RuntimeError(f"Could not upload {path}: {upload.error}")
    ...
    if getattr(updated, "error", None):
        raise RuntimeError(f"Could not finalize E2E video job: {updated.error}")
    print(f"VIDEO_JOB_ID={job_id}")
    return job_id, [teacher_path, visual_path]


def validate_video(output_path: str) -> dict:
    raw = supabase.storage.from_(BUCKET).download(output_path)
    video_path = ROOT / ".e2e-final.mp4"
    ...
    if duration < 1 or size < 100_000 or "mp4" not in format_name:
        raise RuntimeError(f"Rendered artifact is not credible: {metadata}")
...
def main() -> None:
    ...
    if health.get("status") != "ok":
        raise RuntimeError(f"Jupiter health check failed: {health}")

    analysis_job_id = start_analysis(fixture)
    analysis_job = wait_for_job(analysis_job_id, 90 * 60, "analysis")
    analysis = load_analysis(analysis_job)

    video_job_id, artifact_paths = create_render_job(analysis_job_id, analysis)
    cleanup_paths.extend(artifact_paths)
    ...
    report = {
        "workflow": "autonomous-e2e-1",
        "passed": True,
        "base_url": BASE_URL,
        "analysis_job_id": analysis_job_id,
        "video_job_id": video_job_id,
        "analysis_result_path": analysis_job.get("result_path"),
        "video_output_path": output_path,
        "video": video,
        "fixture": {"type": "pdf", "bytes": len(fixture)},
        "elapsed_seconds": round(time.time() - started, 2),
    }
    REPORT.write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(json.dumps(report, indent=2))

    # Keep the final video/result for manual inspection. Remove only temporary
    # input artifacts so repeated E2E runs do not accumulate intermediate files.
...
if __name__ == "__main__":
    try:
        ...
        REPORT.write_text(
            json.dumps({"workflow": "autonomous-e2e-1", "passed": False, "error": str(error)}, indent=2),
            encoding="utf-8",
        )
        raise
